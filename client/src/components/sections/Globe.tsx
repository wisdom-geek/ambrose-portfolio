...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  const _buildData = () => {
    try {
      ...
      for (let i = 0; i < arcs.length; i++) {
        ...
        if (rgb) {
          points.push({
            ...
            order: arc.order,
            color: (t: number) => `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${Math.max(0, Math.min(1, 1 - t))})`,
            lat: arc.startLat,
            lng: arc.startLng,
          });
          points.push({
      ...
      setGlobeData(filteredPoints.length > 0 ? filteredPoints : null);
    } catch (error) {
  ...
  useEffect(() => {
    if (globeRef.current && globeData) {
      try {
        ...
        console.warn("Error configuring globe:", error);
      }
    }
  }, [globeData]);
